...
Todos os objetos tem dois *atributos intrínsecos*: o **modo** e **comprimento**. O **modo** representa a natureza dos elementos objetos. Destacamos quatro, numérico (`numeric`), lógico (logic), caractere^[sinônimo: *string*, cadeia de caracteres.] (`character`) e complexo (`complex`). Este último, não daremos evidência para esse momento. Deixaremos, para uma abordagem completa no Módulo [Programação em R (Nível Intermediário)](nintermediario.html). O **comprimento** mede a quantidade de elementos no objeto.
...
Temos uma outra função para verificarmos o tipo do objeto que é `typeof()`. Essa tipagem de objeto está relacionado a tipagem de variáveis da linguagem `C`, uma vez que boa parte das rotinas no `R` está nessa linguagem, principalmente as funções do pacote `base`. Dentre esses três modos apresentados, usando a função `typeof()`, o modo `numeric` se subdividirá em `integer` e `double`. A diferença entre os dois é o armazenamento de memória ativa. 

Os modos podem ser convertidos do tipo `as.<modo>()`, isto é,...

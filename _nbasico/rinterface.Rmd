...
<iframe width="560" height="315" src="https://www.youtube.com/embed/Dfo8NCyvyGc" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
Vamos inicialmente instalar o pacote [reticulate](https://rstudio.github.io/reticulate/){target="_blank"},

```{r, echo=TRUE, eval=FALSE, prompt=TRUE}
install.packages(reticulate)
```

Existem diversas formas de integração. Vamos pensar na forma mais básica, isto é, temos um [*script Python*](scripts/aula24/add.py){target="_blank"}, isto é, um arquivo com a extensão *.py*, e vamos carregá-lo no `r rlink`. O *script*, chamaremos de [add.py](scripts/aula24/add.py){target="_blank"}, que segue:
...
Vamos agora chamar a função `add()` em um [*script R*](scripts/aula24/r-python.R){target="_blank"}:
...
Vamos instalar o pacote [Rcpp](http://www.rcpp.org/){target="_blank"},

```{r, echo=TRUE, eval=FALSE, prompt=TRUE}
install.packages(Rcpp)
```

Vamos realizar a mesma aplicação feita em *Python*, que segue o [*script C++*](scripts/aula24/add.cpp){target="_blank"}, com extensão *.cpp*,
...
Vamos agora chamar a função `add()` em um [*script R*](scripts/aula24/r-rcpp.R){target="_blank"}:
...
Existem outras formas de chamar as rotinas em *Python*, *C++* e *Tcl/Tk* no `r rlink`, implementadas nos pacotes [reticulate](https://rstudio.github.io/reticulate/){target="_blank"}, [Rcpp](http://www.rcpp.org/){target="_blank"} e *tcltk*, respecitvamente. Contudo, iremos apresentá-los no módulo [Programação em R (Nível Avançado)](navancado.html){target="_blank"}, bem como a integração de outras linguagens.
